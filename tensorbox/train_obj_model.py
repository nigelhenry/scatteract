...
def build_lstm_inner(H, lstm_input):
    ...
    if H['num_lstm_layers'] > 1:
        lstm = tf.keras.layers.StackedRNNCells([lstm_cell for _ in range(H['num_lstm_layers'])])
    else:
        lstm = lstm_cell
...
def rezoom(H, pred_boxes, early_feat, early_feat_channels, w_offsets, h_offsets):
    ...
    interp_indices = tf.concat(indices, axis = 0)
...
def build_forward(H, x, googlenet, phase, reuse):
    ...
    if H['deconv']:
        ...
        cnn2 = tf.nn.avg_pool(cnn2, ksize=[1, pool_size, pool_size, 1],
                              strides=[1, 1, 1, 1], padding='SAME')
        cnn = tf.concat([cnn1, cnn2], axis = 3)
    ...
    with tf.compat.v1.variable_scope('decoder', reuse=reuse, initializer=initializer):
        ...
        pred_boxes = tf.concat(pred_boxes, axis = 1)
        ...
        if H['use_rezoom']:
            ...
            for k in range(H['rnn_len']):
                delta_features = tf.concat([lstm_outputs[k], rezoom_features[:, k, :] / 1000.], axis = 1)
                dim = 128
                ...
                pred_confs_deltas.append(tf.reshape(tf.matmul(ip1, delta_confs_weights) * scale,
                                                    [outer_size, 1, H['num_classes']]))
            pred_confs_deltas = tf.concat(pred_confs_deltas, axis = 1)
            if H['reregress']:
                pred_boxes_deltas = tf.concat(pred_boxes_deltas, axis = 1)
